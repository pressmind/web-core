...
class DocumentMediaObject extends AbstractObject
{
    ...
    public function getImageSrc($derivativeName)
    {
        ...
        if(!isset($config['imageprocessor']['derivatives'][$derivativeName])) {
        ...
        if($derivative = $this->hasDerivative($derivativeName)) {
            ...
            $uri = $config['imageprocessor']['image_http_path'] . $this->_generateDerivative($derivativeName)->file_name;
        }
        if($config['imageprocessor']['webp_support'] == true && $config['imageprocessor']['derivatives'][$derivativeName]['webp_create'] == true && defined('WEBP_SUPPORT') && WEBP_SUPPORT === true) {
            ...
            if(file_exists(BASE_PATH . DIRECTORY_SEPARATOR . $config['imageprocessor']['image_file_path'] . DIRECTORY_SEPARATOR . str_replace($path_info['extension'], 'webp', $derivative->file_name))) {
    ...
    {
        $config = Registry::getInstance()->get('config');
        $derivative_config = Config::create($derivativeName, $config['imageprocessor']['derivatives'][$derivativeName]);
        ...
        $newname = $download->download($tmp_url, BASE_PATH . DIRECTORY_SEPARATOR . $config['imageprocessor']['image_file_path'], 'itinerary_' . $this->id_media_object. '_' . $derivativeName);
        ...
        $webp_processor->process($derivative_config, BASE_PATH . DIRECTORY_SEPARATOR . $config['imageprocessor']['image_file_path'] . DIRECTORY_SEPARATOR . str_replace('_' . $derivativeName, '', $derivative->file_name), $derivative_config->name);
        return $derivative;
    }
}
