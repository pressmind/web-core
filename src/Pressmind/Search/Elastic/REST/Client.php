...
class Client
{
    ...
    public function sendRequest($controller, $action, $params = null) {
        Writer::write('CURL initialized', Writer::OUTPUT_FILE, $this->_logfile_name . '.log');
        $ch = curl_init();
        if(is_array($params)) {
            $params['cache'] = 0;
        } else {
        ...
        Writer::write('Sending request to: ' . $this->_api_endpoint . $this->_api_key . '/' . $controller . '/' . $action . $get_params, Writer::OUTPUT_FILE, $this->_logfile_name . '.log');
        $response = curl_exec($ch);
        if($response === false) {
            Writer::write('Got no response from: ' . $this->_api_endpoint . $this->_api_key . '/' . $controller . '/' . $action . $get_params, Writer::OUTPUT_FILE, $this->_logfile_name . '_error.log');
            throw new Exception(curl_error($ch));
        } else {
            Writer::write('Got response from: ' . $this->_api_endpoint . $this->_api_key . '/' . $controller . '/' . $action . $get_params, Writer::OUTPUT_FILE, $this->_logfile_name . '.log');
            Writer::write('Parsing response data ...', Writer::OUTPUT_FILE, $this->_logfile_name . '.log');
        }
        ...
        if(is_null($json)) {
            ...
            Writer::write('Response decoding for: ' . $this->_api_endpoint . $this->_api_key . '/' . $controller . '/' . $action . $get_params . ' failed: ' . $error_msg, Writer::OUTPUT_FILE, $this->_logfile_name . '_error.log');
            throw new Exception($error_msg);
        } else {
            Writer::write('Response data successfully parsed.', Writer::OUTPUT_FILE, $this->_logfile_name . '.log');
        }
        return $json;
